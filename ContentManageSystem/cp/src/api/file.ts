...
export const downloadBackupDatabase = async (dir: string, names: string[]): Promise<any> =>
    (await axios.post('/backend/ext/backup-database/download-zip', { dir, names }, { responseType: 'blob' })).data;
export const backupBackupDatabase = async (): Promise<any> => (await axios.post('/backend/ext/backup-database')).data;
export const restoreBackupDatabase = async (name: string): Promise<any> => (await axios.put(`/backend/ext/backup-database/restore?name=${name}`)).data;
